...
if ! curl -sL https://api.github.com/repos/wnxd/homebrew-v2raya/releases/latest > /tmp/v2raya.json; then
...
if [ "$current_version" != "$latest_version" ]; then
    if ! curl -sL https://github.com/wnxd/homebrew-v2raya/releases/download/$latest_version/v2raya-x86_64-linux.zip.sha256.txt > /tmp/v2raya-linux.sha256; then
        echo "GitHub API rate limit exceeded, please try again later."
        exit
    else
        latest_sha_linux_x64="$(cat /tmp/v2raya-linux.sha256 | awk '{print $1}')"
    fi
    if ! curl -sL https://github.com/wnxd/homebrew-v2raya/releases/download/$latest_version/v2raya-x86_64-macos.zip.sha256.txt > /tmp/v2raya-macos.sha256; then
        echo "GitHub API rate limit exceeded, please try again later."
        exit
    else
        latest_sha_macos_x64="$(cat /tmp/v2raya-macos.sha256 | awk '{print $1}')"
    fi
    if ! curl -Ls https://github.com/wnxd/homebrew-v2raya/releases/download/$latest_version/v2raya-aarch64-macos.zip.sha256.txt > /tmp/v2raya-macos-arm64.sha256; then
        echo "GitHub API rate limit exceeded, please try again later."
        exit
    else
